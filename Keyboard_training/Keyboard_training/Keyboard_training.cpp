...
// прототипы функций:
void options(HANDLE h); // настройка размеров и расположения окна, установка заголовка, скрытие мигающего курсора
void intro(HANDLE h); // заставка
void menu(HANDLE h); // три пункта меню
void letter(HANDLE h, int x, int y, int forecolor, int backcolor, char symb); // рисование квадрата с буквой для заставки
void text_effect(HANDLE h, int x, int y, int forecolor, int backcolor, const char* symb, int len, int pause); // показ "ленты" текста
void exit(HANDLE h); // быстрый выход из приложения
void the_end(HANDLE h); // концовка с пожеланием удачи
void stand(HANDLE h, int x, int y, int k, const char* str); // вспомогательная функция по установке курсора в нужную позцию + окрашивание
void help(HANDLE h); // открытие файла справки
void epic_win(HANDLE h); // поздравляшки, если удалось набрать 100 очков (это реально!)
void game(HANDLE h); // основной игровой процесс
...
void letter(HANDLE h, int x, int y, int forecolor, int backcolor, char symb) // функция вывода квадратика с буквой (стилизация под клавиатуру)
{
...
void hideConsoleCursor(HANDLE h) {
...
void text_effect(HANDLE h, int x, int y, int forecolor, int backcolor, const char* text, int len, int pause)
{
	for (int i = 0; i < len; i++)
	{
...
void the_end(HANDLE h)
{
	text_effect(h, 5, 2, 12, 0, "Спасибо за игру!", 16, 15); // красным цветом благодарим за игру
	text_effect(h, 5, 4, 7, 0, "Для выхода нажмите Enter", 24, 15); // белым цветом выводим просьбу нажать на энтер
...
void intro(HANDLE h)
{
	...
	for (int i = 0; i < ARRAYSIZE(message) - 1; i++) {
		int fore = 0;
		int back = rand() % 8 + 1;
		letter(h, 5 + i * 10, 4, fore, back, message[i]);
	...
	for (int i = 0; i < ARRAYSIZE(message2) - 1; i++)
	{
		int fore = 0;
		int back = rand() % 8 + 1;
		letter(h, 9 + i * 10, 12, fore, back, message2[i]);
		Sleep(100);
	}
	text_effect(h, 56, 22, 15, 0, "Для начала тренировки нажмите 'Y'", 33, 30);
	text_effect(h, 5, 46, 7, 0, "Для выхода из программы нажмите 'N'", 36, 15);

	while (true)
	{
		int n = _getch(); // анализируем код клавиши
		if (n == 'N' || n == 'n') exit(h); // выходим
		if (n == 'Y' || n == 'y') break; // продолжаем игровой процесс - возвращаемся в мейн
	}
}

void stand(HANDLE h, int x, int y, int k, const char* str)
{
	COORD c{ x, y };
...
void epic_win(HANDLE h)
{
	text_effect(h, 5, 2, 12, 0, "Вы победили!", 16, 15);
	text_effect(h, 5, 4, 7, 0, "Для выхода нажмите Enter", 24, 15);
	...
	{ // пока не проиграем
		if (_kbhit()) // если было нажатие на клавиши пользователем
		{
			...
			{ // если мы нажали именно на ту клавишу, аски-код которой соответствует падающей букве
				score++; // +1 к очкам
				if (score >= 100) epic_win(h); // если набрали 100 очок - победа
		...
		{ // если нажатия не было - то буква просто падает
			...
			{ // если буква опустилась слишком низко...
				score--; // - одно очко
				if (score < 2) the_end(h); // если меньше двух очок - печаль!
...
void menu(HANDLE h)
{
	...
	setlocale(0, "RUS");
	stand(h, 5, start_y, 10, "Справка");
	stand(h, 5, start_y + 2, 2, "Игра");
	stand(h, 5, start_y + 4, 2, "Выход");
	...
	while (true)
	{
		...
		if (k == 80) // если нажата курсорная клавиша вниз
		{
			if (cur < 3)
			{
				...
				if (cur == 2)
				{
					stand(h, 5, start_y, 2, "Справка"); // 2 - это тёмно-зелёный цвет
					stand(h, 5, start_y + 2, 10, "Игра"); // 10 - ярко-зелёный
				}
				else if (cur == 3) {
					stand(h, 5, start_y + 2, 2, "Игра");
					stand(h, 5, start_y + 4, 10, "Выход");
				}
			}
		}
